...
## TF Notes
The core concept students needed to understand is that RAG quality is bottlenecked by retrieval — even a well-prompted LLM cannot compensate for a retrieval step that returns the wrong chunk or nothing at all. Students most commonly struggle with the score threshold: it's tempting to lower it to get more results, but that floods the LLM with irrelevant noise and produces confidently wrong answers. A subtler struggle is understanding why term-overlap scoring fails on synonyms (e.g. "lists" vs. "returns") — students often assume retrieval is working correctly and blame the LLM prompt instead. AI was genuinely helpful for drafting the RAG prompt, especially the explicit refusal instruction ("I do not know based on the docs I have"), but misleading when it suggested removing the score threshold entirely, which would have degraded reliability. If a student's RAG mode is refusing too aggressively, I'd ask them to log what retrieve() actually returns for that query before touching the prompt — this usually reveals whether the problem is in retrieval or generation.
